...
def menu(message):
    ...
    if user_status == 'admin':
        bot.send_message(text="Администратор", parse_mode='HTML', chat_id=message.chat.id, reply_markup=admin_keyboard())
    elif user_status == 'specialist':
        bot.send_message(text="Специалист", parse_mode='HTML', chat_id=message.chat.id, reply_markup=specialist_keyboard())
...
def add_client(message):
    ...
    try:
        ...
        sql = "INSERT INTO clients (phone, status, date, time, admin_id, note, car_info) VALUES (%s,%s,%s,%s,%s,%s,%s)"
        val = (phone, 0, date, time, message.chat.id, note, car_info)
...
def my_clients_adm(message):
    ...
    for row in data:
        ...
        bot.send_message(text="#{}\n"
                              ...
                              "Телефон: {}\n"
                              "Марка и модель: {}\n"
                              "Примечание: {}\n".format(counter,row[0],row[3].strftime("%Y-%m-%d"), str(row[4]), str(spec_name).replace('None',''), str(adm_name).replace('None',''), row[5], str(row[11]).replace('None',''), str(row[10]).replace('None','')),
                         chat_id=message.message.chat.id,
...
def accept_order(message):
    ...
    if user_status == 'specialist':
        ...
        bot.send_message(text="Клиента №{} принял специалист {}\n@{}".format(client_id,spec_local_name,spec_user_name),
                         chat_id=admin_chat_id)
...
def make_deal_approve(message, client_id):
    price = message.text
    bot.send_message(text='Клиент покупает авто?', chat_id=message.chat.id,reply_markup=deal_keyboard(client_id,price))
...
def make_deal(message):
    ...
    if deals_amount == 0:
        ...
        db_cursor.execute(sql)
        db.commit()
        bot.send_message(text="Ваш баланс увеличен на 300", chat_id=adm_id)
    ...
    if buy_auto == 0:
        bot.send_message(text="Сделка создана. Клиент остается в работе.", chat_id=message.message.chat.id, reply_markup=return_keyboard())
    else:
...
def refuse_client(message):
    ...
    bot.register_next_step_handler(message=message.message, callback=refuse_comment, client_id=client_id, admin_id=admin_id)
...
def get_payment(message):
    ...
    bot.send_message(text="Администратор {} запрашивает выплату\n\n"
                          "Количество сделок: {}\n"
                          "Баланс: {}\n".format(admin_name,admin_balance//3,admin_balance),
                     chat_id=560544363, reply_markup=approve_payment_kb(message.message.chat.id))
...
@bot.callback_query_handler(func=lambda message: message.data == 'send_order_to_chat')
...
def set_spec_from_admin(message):
    ...
    bot.send_message(text="Вам назначен клиент №{}".format(client_id),
                     chat_id=message.message.chat.id
                     )
...
def set_spec_keyboard(client_id):
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.add(telebot.types.InlineKeyboardButton(text="Отправить заказ в чат", callback_data='send_order_to_chat'))
...
def base_client_keyboard(client_id):
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.add(telebot.types.InlineKeyboardButton(text="Начать работу с клиентом", callback_data='start_work_with_client_{}'.format(client_id)))
    keyboard.add(telebot.types.InlineKeyboardButton(text="Назначить специалиста", callback_data="set_specialist_{}".format(client_id)))
    return keyboard
...
def approve_payment_kb(admin_id):
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.add(telebot.types.InlineKeyboardButton(text="Подтвердить выплату", callback_data='approve_payment_{}'.format(admin_id)))
    return keyboard
...
def client_keyboard_adm(client_id):
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.add(telebot.types.InlineKeyboardButton(text="⏳ Получить историю клиента", callback_data='client_history_{}'.format(client_id)))
    keyboard.add(telebot.types.InlineKeyboardButton(text="💬 Добавить комментарий", callback_data='add_comment_{}'.format(client_id)))
...
def client_keyboard_spec(client_id):
    ...
    keyboard.add(telebot.types.InlineKeyboardButton(text="💬 Добавить комментарий", callback_data='add_comment_{}'.format(client_id)))
    keyboard.add(telebot.types.InlineKeyboardButton(text='❌ Отказ', callback_data='refuse_{}'.format(client_id)))
    keyboard.add(telebot.types.InlineKeyboardButton(text="🤝 Создать сделку", callback_data='make_deal_{}'.format(client_id)))
    return keyboard
...
def admin_keyboard():
    ...
    keyboard.add(telebot.types.InlineKeyboardButton(text="⏳💰 Получить историю выплат", callback_data='get_payment_history'))
    return keyboard
...
def locations_keyboard():
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.add(telebot.types.InlineKeyboardButton(text="🏙 Москва", callback_data='city_moscow'))
    keyboard.add(telebot.types.InlineKeyboardButton(text="🏙 Санкт-Петербург", callback_data='city_petersburg'))
    keyboard.add(telebot.types.InlineKeyboardButton(text="Тест чат", callback_data='test_chat'))
    return keyboard
